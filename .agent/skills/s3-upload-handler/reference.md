...
## Best Practices
- **Data Isolation**: Always prefix S3 keys with `organizationId` or `userId` to prevent collisions and ensure data privacy.
- **Validation**: Validate file types and sizes both on client (for UX) and server (presigned URL conditions).
