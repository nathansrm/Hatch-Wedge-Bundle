...
- **Association**: Plans are linked to **Organizations** (`organizations.planId`), not Users.
...
- **Usage**: `getSubscribeUrl({ codename, type, provider, organizationId })`.
- **Context**: Ensure `organizationId` is passed to the checkout session to link the subscription correctly.
