...
3.  **Sending Logic**: `src/lib/email/sendMail.ts` is a **stub** — implement your provider per [docs](https://docs.indiekit.pro/setup/email). Use `render` from `@react-email/components` to produce HTML, then call `sendMail`.
4.  **Organization Branding**: Use organization context (name, logo) if the email is sent on behalf of a tenant.
...
- Accept props for dynamic data (e.g., `name`, `url`, `expiresAt`, `organizationName`).
...
export default function MyEmail({ username, actionUrl, organizationName }: MyEmailProps) {
  return (
    <Html>
      <Layout previewText="Action Required">
        <Text>Hi {username},</Text>
        {organizationName && <Text>You have been invited to join {organizationName}.</Text>}
...
const html = await render(
  MyEmail({ 
    ...
    organizationName: organization.name // Pass from context
  })
);
