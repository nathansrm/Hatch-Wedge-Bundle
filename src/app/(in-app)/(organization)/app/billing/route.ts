...
export const GET = withOrganizationAuthRequired(async (req, context) => {
  ...
  if (process.env.POLAR_ACCESS_TOKEN && organization.polarCustomerId) {
    try {
      ...
      console.error("Polar customer portal redirect failed — continuing fallback");
    }
  }

  const dodoCustomerId = organization.dodoCustomerId;
  if (dodoCustomerId) {
    const customerPortalSession = await getDodoClient().customers.customerPortal.create(
      dodoCustomerId
    );
    return redirect(customerPortalSession.link);
  }

  const paddleCustomerId = organization.paddleCustomerId;
  if (paddleCustomerId) {
    try {
      const portalSession =
      ...
      console.error(
        "Unable to create Paddle customer portal session — continuing fallback."
      );
    }
  }
