...
import { ensurePolarCustomerIdForOrganization } from "@/lib/polar/ensureCustomer";
...
async function SubscribePage({
  ...
  switch (provider) {
    ...
    case PlanProvider.PADDLE: {
      ...
      if (currentOrganization.paddleSubscriptionId) {
        if (type === PlanType.ONETIME) {
          return redirect(
      ...
      const paddleCheckout = await createPaddleCheckout({
        organization: {
          ...
          paddleCustomerId: currentOrganization.paddleCustomerId,
        },
      ...
      throw new Error("Paddle checkout could not be created");
    }
    ...
    case PlanProvider.POLAR: {
      ...
      if (currentOrganization.polarSubscriptionId) {
        if (type === PlanType.ONETIME) {
          return redirect(
      ...
      const polarCustomerId = await ensurePolarCustomerIdForOrganization({
        polar,
